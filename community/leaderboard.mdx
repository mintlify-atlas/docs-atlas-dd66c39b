...
<Tabs>
  ...
  <Tab title="CLI: summary">
    ```bash
    obliteratus aggregate --format summary
    ```

    Prints a ranked table to stdout showing method, model, run count, and key metrics.
  </Tab>
  ...
  <Tab title="CLI: LaTeX">
    ...
    Outputs a LaTeX `tabular` environment suitable for direct inclusion in a paper. The `--min-runs 3` flag filters out model/method combinations with fewer than three contributing runs, reducing noise.

    Available `--metric` values: `refusal_rate`, `perplexity`, `coherence`.
  </Tab>
</Tabs>
...
obliteratus aggregate --dir ./my_results --format summary

# Generate a LaTeX table filtered to refusal_rate with at least 3 runs
obliteratus aggregate --format latex --metric refusal_rate --min-runs 3

# Generate a LaTeX table for coherence
obliteratus aggregate --format latex --metric coherence
```
...
<AccordionGroup>
  ...
  <Accordion title="How much weight should I give to runs with n=1?">
    Single-run entries (`runs: 1`) have no variance estimate and may reflect hardware-specific anomalies. Look for entries with `runs >= 3` for reliable conclusions. Use `--min-runs 3` with the `aggregate` command to filter automatically.
  </Accordion>
