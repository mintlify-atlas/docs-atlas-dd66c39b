...
description: "Compare all seven obliteration methods to pick the right one for your use case."
---

OBLITERATUS ships with seven weight-projection presets and one analysis-informed pipeline. Each preset is a named bundle of parameters in the `METHODS` dict — you select one with `--method` on the CLI or `method=` in the Python API.
